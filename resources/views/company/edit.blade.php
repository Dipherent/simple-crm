    ...
    <div class="relative max-w-7xl mx-auto lg:grid lg:grid-cols-5">
      <div class="bg-gray-50 py-16 px-4 sm:px-6 lg:col-span-2 lg:px-8 lg:py-24 xl:pr-12">
        <div class="max-w-lg mx-auto">
          <h2 class="text-2xl leading-8 font-extrabold tracking-normal text-gray-900 sm:text-3xl sm:leading-9">
             <span class="border-b-8 border-red-500">Edit</span>  Company
          </h2>
      ...
      <div class="bg-white py-16 px-4 sm:px-6 lg:col-span-3 lg:py-24 lg:px-8 xl:pl-12">
        <div class="max-w-lg mx-auto lg:max-w-none">
          <form action="/contacts/{{$company->id}}" method="POST" class="grid grid-cols-1 gap-y-6">
            @csrf
            @method('PATCH')
            <div>
              <label for="name" class="sr-only">Company name</label>
              <div class="relative rounded-md shadow-sm">
              <input value="{{ $company->name }}" type="text" name="name" class="form-input block w-full py-3 px-4 placeholder-gray-500 transition ease-in-out duration-150" placeholder="Company name">
                    @error('name') 
            ...
            <div>
              ...
              <div class="relative rounded-md shadow-sm">
                <input value="{{ $company->address  }}" name="address" type="text" class="form-input block w-full py-3 px-4 placeholder-gray-500 transition ease-in-out duration-150" placeholder="Email">
                @error('address') 
                <p class="rounded-md bg-red-100 p-2 mt-2 text-sm leading-5 text-red-700"> {{$message}}</p>
            @enderror
              </div>
            ...
            <div>
              ...
              <div class="relative rounded-md shadow-sm">
                <textarea name="notes" type="text" rows="4" class="form-input block w-full py-3 px-4 placeholder-gray-500 transition ease-in-out duration-150" placeholder="Notes">{{ $company->notes }}</textarea>
              </div>
            </div>
